...
void turnHandler(int sig, siginfo_t* infoPtr, void* dataPtr){

  pid_t joeyPid = infoPtr->si_pid;
  int joey = getJoeyIndex(joeyPid);
  
  if ( !wasJoeysTurnCorrect(joey,sig) ){
    printf("Mall: Joey # %d made the wrong turn!  (%s)\n",joey,sig == LEFT_TURN_SIGNAL ? "left" : "right");
    kill(joeyPid,WRONG_TURN_SIGNAL);
  }
  else {
    printf("Mall: Joey # %d turned correctly!  (%s)\n",joey,sig == LEFT_TURN_SIGNAL ? "left" : "right");
    joeysNumSuccessfulTurns[joey]++;
    if (joeysNumSuccessfulTurns[joey] >= NUM_TURNS_MUST_SUCCESSFULLY_MAKE){
      printf("Mall: Joey # %d got out of the mall and returned to its mama!\n",joey);
      kill(joeyPid,SIGINT);
    }
    else{
      printf("Mall: Joey # %d must make %d more correct turns.\n",joey,NUM_TURNS_MUST_SUCCESSFULLY_MAKE - joeysNumSuccessfulTurns[joey]);
      kill(joeyPid,CORRECT_TURN_SIGNAL);
    }
  }
